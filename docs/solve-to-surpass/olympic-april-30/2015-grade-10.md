...
Để tính `s1` là tổng chi phí kết nối nhỏ nhất, ta áp dụng thuật toán Krsuskal, với mỗi công ty là một đỉnh và mỗi kết nối giữa hai công ty là một cạnh.

Để tỉnh `s2` là tổng chi phí nhỏ thứ nhì, ta thử lần lượt:

- Bỏ bớt một cạnh trong cây khung nhỏ nhất có được khi tính `s1`

- Thực hiện lại Krsuskal với những cạnh còn lại để tìm ra `s2` nào vừa đủ lớn hơn hoặc bằng `s1`.

Hinh sau minh hoạ công đoạn thực hiện Krsuskal:

![Minh hoạ công đoạn thực hiện Krsuskal](./images/2015-olympic10-april-30-mangdien.svg){loading=lazy}
...
Khai báo mảng `rep` và `ranking` để áp dụng cấu trúc disjoint-set trong thuật toán Krsuskal.
...
**Bước 1:** Viết hàm thực thi Krsuskal

Hàm `krsuskal()` gồm hai tham số:
...
Giả sử các kết nối đã được sắp xếp theo chi phí tăng dần. Hàm `kruskal()` thực hiện như sau:
...
Hàm `process()` gọi `kruskal()` lần thứ nhất để tính chi phí kết nối nhỏ nhất `s1`, đồng thời cũng trả về các kết nối được chọn cho cây khung nhỏ nhất, thông qua mảng `included_edges`.

Trước khi gọi `kruskal()`, các kết nối cần được sắp xếp theo chi phí tăng dần.
...
- Gọi hàm `kruskal()` để tính chi phí của cây khung nhỏ nhất mới, đặt là `cost`.
